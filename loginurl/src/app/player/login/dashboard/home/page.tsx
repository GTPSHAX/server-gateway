...
export default async function Page({
  ...
  const data = decodeURIComponent((await searchParams)?.data as string) ?? "";
    ...
    <div className='w-screen h-screen flex justify-center items-center scale-y-85 bg-transparent bg-none'>
      <Dialog style='default' className={`p-2 bg-[#226f8de8] ${century_gothic_bold.className}`}>
        ...
        <form 
          ...
          id="loginForm"
        >
          <input type="hidden" name="_token" defaultValue={data as string} />
          {/* <input type="hidden" name="session" defaultValue={sessionCookie} /> */}
          <input type="hidden" name="merchant_name" defaultValue={merchant_name as string} />
          <input type="hidden" name="is_guest" value="false" />

          <div className="flex flex-col gap-2">
            <label className='text-sm text-[#fce6ba] growid' htmlFor="loginGrowId">
              GrowID:
              <Dialog style="default" className="w-full input">
                <input
                  type="text"
                  ...
                  name="growId"
                  id="loginGrowId"
                  defaultValue={pData["tankIDName"] ?? ''}
                  required
                />
              </Dialog>
            </label>
            <label className='text-sm text-[#fce6ba] password' htmlFor="loginPassword">
              Password:
              <Dialog style="default" className="w-full input">
                <input
                  type="password"
        ...
        {/* Form terpisah untuk Guest Mode */}
        <form 
          method="POST"
          action="/player/growid/login/validate"
          acceptCharset="UTF-8"
